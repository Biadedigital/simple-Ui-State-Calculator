## simple State Calculator UI

using React UseState to Change UI Without using the native maths function 

## Tools
- [React](https://reactjs.org/)
- [Vite](https://vitejs.dev/)
- [Replit](https://replit.com/@Biademade)
- [Github](https://github.com/Biadedigital)

## WebView
- [Click-Me](https://simple-Ui-State-Calculator.biademade.repl.co)


## Code
- [Code-on-Replit](https://replit.com/@Biademade/simple-Ui-State-Calculator?v=1)
- [Code-on-Github](https://github.com/Biadedigital/simple-Ui-State-Calculator)